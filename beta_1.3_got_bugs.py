...
def filter_get_artifact(a, name, main_stat, c=0, d=20):
    ...
    main_stat_chance = get_main_stat_chance(name)
...
def predict_and_change_to_list(a, threshold):
    result = []
    for dict1 in a:
        threshold_probability =  1/(5*300*main_stat_chance)
        prediction_list = new_predict(dict1, threshold, threshold_probability)
        probability = "The probablity of achieving " + str(threshold) + "rolls is: " + str(prediction_list[0]*100) + "%"
        probable_value = "The Median rolls achiveable is: " + str(prediction_list[1]) + "Rolls" 
        plausible_ceiling = "The potential ceiling is: "  + prediction_list[2] + "Rolls with " + str(threshold_probability)#    result = [probability,probable_value, plausible_ceiling]
        values = [dict1['setKey'], dict1['slotKey'], dict1['rarity'], dict1['mainStatKey'], dict1['level'], dict1['location'], dict1['lock'], dict1['id'], dict1['substats'], probability, probable_value, plausible_ceiling]
        result.append(values)
    print(f"Done conversion of dicts to lists\n")
    sorted_result = sorted(result, key=lambda x: x[8], reverse=True)
    print(f"Sorting of lists successfull\n")
